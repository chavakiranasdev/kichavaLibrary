public class Solution
{
    ...
    var rowAvailableList = new Dictionary<int, List<int>>();
    var columnAvailableList = new Dictionary<int, List<int>>();
    var gridAvailableList = new Dictionary<int, List<int>>();
    ...
    public void SolveSudoku(char[][] board)
    {
            ...
            while (currentColumn < SizeOfSudoku)
            {
                ...
                if (currentCell.CurrentValue != 0)
                {
                    ...
                    else
                    {
                        ...
                        RemoveCellValue(currentRow, currentValue);
                    }
                }
        ...
        for (int i = 0; i < SizeOfSudoku; i++)
        {
            ...
            {
                board[i][j] = cellBoard[i][j].CurrentValue;
            }
        }
    }
    ...
    private void RemoveCellValue(int row, int column)
    {
        ...
        ResetAvailableListsFromCellBoardFor(i, j, currentValue);
    }
    ...
    {
        cellBoard[row][column].CurrentValue = currentValue;
        ...
        UpdateAvailableListsFromCellBoardFor(i, j);
    }
    ...
    private int GetBestGuess(int row, int column)
    {
        ...
        if (count != 0)
        {
            ...
            return availableInAllThree[random.Next(0, count)];
        }
    ...
    private void InitializeCellArray(char[][] board)
    {
        for (int i = 0; i < SizeOfSudoku; i++)
        {
            for (int j = 0; j < SizeOfSudoku; j++)
            {
                ...
                if (board[i][j] != '.')
                {
                    UpdateCellValue(i, j, board[i][j], true, false, false);
                }
            }
        }
    }
    ...
    internal class Cell
    {
        ...
        bool IsInitialValue { get; set; }
        bool IsSolvedWithConfidence { get; set; }
        bool IsInProgress { get; set; }
        List<int> NotFitForThisCell { get; set; } // We tried this number for this cell and think this may not fit here.
